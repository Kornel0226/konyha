            ...
            <div className="w-full bg-orange-400 h-10 flex flex-row items-center pl-2 pr-2 border-l-2 border-r-2 border-black">
                ...
                <menu className={`w-full bg-orange-400 text-2xl font-bold ${isMenuOpen ? "block absolute top-full left-0 z-50 border-black border-x-2 border-t-2" : "hidden"} md:block md:flex-col`}>
                    <ul className={`flex flex-row ${isMenuOpen ? "flex-col" : "gap-6"}`}>
                        ...
                        <li className={`${isMenuOpen ? "text-center border-b-2 border-black py-2" : ""}`} onClick={() => setIsMenuOpen(false)}><NavLink to="/receptek">Receptek</NavLink></li>
                        {!user && <li className={`${isMenuOpen ? "text-center border-b-2 border-black py-2" : "ml-auto"}`} onClick={() => setIsMenuOpen(false)}><NavLink to="/login">BEJELENTKEZES</NavLink></li>}
                        {!user && <li className={`${isMenuOpen ? "text-center border-b-2 border-black py-2" : ""}`} onClick={() => setIsMenuOpen(false)}><NavLink to="/register">REGISZTRACIO</NavLink></li>}
                        {user && <UserNavItem />}
                    </ul>
                </menu>
            </div>
